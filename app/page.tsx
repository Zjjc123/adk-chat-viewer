...
import { Box, Container, Title, Text, Paper, Avatar, Divider, Alert, Group, Button, Card, Collapse } from "@mantine/core";
import { IconAlertCircle, IconDownload, IconFile, IconFileText, IconChevronDown, IconChevronRight } from "@tabler/icons-react";
...
export default function ADKPage() {
    ...
    const extractInlineData = (part: any): InlineData | null => {
        if (part && part.inlineData && part.inlineData.displayName && part.inlineData.data) {
            console.log("Found inlineData with displayName:", part.inlineData.displayName);
            return {
    ...
    useEffect(() => {
        if (chatSession && chatSession.events) {
                    ...
                    event.content.parts.forEach((part: any) => {
                        ...
                        if (part.functionResponse && part.functionResponse.id) {
                            if (callMap[part.functionResponse.id]) {
                                callMap[part.functionResponse.id].response = part.functionResponse.response;
                            } else {
            ...
            const extractedMessages = chatSession.events.map(event => {
                ...
                if (event.content && event.content.parts) {
                    ...
                    isResponseOnlyMessage = event.content.parts.every(part =>
                        part.functionResponse && !part.functionCall && !part.text && !part.inlineData
                    );
                ...
                return {
                    ...
                    functionCalls: callsWithResponses.length > 0 ? callsWithResponses : undefined
                };
    ...
    const handleDrop = async (files: File[]) => {
        ...
        try {
            ...
            setError("Failed to parse JSON file. Please ensure it's a valid ADK session file.");
    ...
    const renderInlineFiles = (files: InlineData[]) => {
        return (
            <Box mt="md" mb="md">
                <Text size="sm" fw={500} mb="xs">Attached Files:</Text>
                <Group>
                    {files.map((file, index) => {
                        const isPdf = file.mimeType === 'application/pdf' || file.displayName.toLowerCase().endsWith('.pdf');
    ...
    const renderFunctionCalls = (functionCalls: FunctionCall[]) => {
        return (
            <Box mt="md" mb="md">
                <Text size="sm" fw={500} mb="xs">Function Calls:</Text>
                        ...
                        <Paper key={index} withBorder p="xs" mb="xs">
                            ...
                            <Collapse in={isExpanded}>
                                <Box mt="xs">
                                    ...
                                    <Paper bg="gray.1" p="xs" mt="xs" style={{ fontFamily: 'monospace', fontSize: '12px' }}>
                                    ...
                                    {call.response && (
                                        <>
                                            ...
                                            <Paper bg="gray.1" p="xs" mt="xs" style={{ maxHeight: '300px', overflow: 'auto' }}>
                                                {typeof call.response === 'object' ? (
                                                    <Box className="markdown-content">
                                                        <ReactMarkdown>
                                                            {call.response.result || JSON.stringify(call.response, null, 2)}
                                                        </ReactMarkdown>
                                                    </Box>
                                                ) : (
    ...
    return (
        <Container size="lg" py="xl">
            <Title order={1} mb="lg">ADK Chat History Viewer</Title>

            {error && (
                <Alert icon={<IconAlertCircle size="1rem" />} title="Error" color="red" mb="md" onClose={() => setError(null)}>
                    {error}
                </Alert>
            )}
            ...
            {!chatSession && (
                <Box mb="xl">
                    <Text mb="md">Upload your ADK session JSON file to view the chat history.</Text>
                    <FileUploadDropzone
                        onDrop={handleDrop}
            ...
            {chatSession && (
                <>
                    ...
                    {messages.length > 0 && (
                        <Box>
                            <Title order={2} mb="md">Chat History</Title>
                                            ...
                                            <Box style={{ width: 'calc(100% - 50px)' }}>
                                                <Text fw={500} mb="xs">
                                                    {message.role === "user" ? "User" :
                                                        (message.author && message.author !== "user" ? message.author : "ADK Agent")}
                                                    {message.timestamp && <Text span size="xs" c="dimmed" ml="xs">
                                                ...
                                                {message.inlineFiles && message.inlineFiles.length > 0 &&
                    ...
                    {messages.length === 0 && (
                        <Alert icon={<IconAlertCircle size="1rem" />} title="No Content Found" color="yellow" mb="md">
